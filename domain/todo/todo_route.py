...
def getTodoList(
    ...
    try:
        ...
        total, todoList = todo_crud.getTodoList(
            db=db, userId=userId, skip=page * size, limit=size
        )
        if todoList is None:
            raise credentialsException
...
def getTodoDetail1(
    todoId: int, token=Depends(oauth2Scheme), db: Session = Depends(get_db)
):
    credentialsException = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="토큰이 유효하지 않습니다.",
...
def createTodo(
    schema: CreateTodo, token=Depends(oauth2Scheme), db: Session = Depends(get_db)
):
    credentialsException = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="토큰이 유효하지 않습니다.",
...
def updateTodo(
    schema: UpdateTodo, token=Depends(oauth2Scheme), db: Session = Depends(get_db)
):
    credentialsException = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="토큰이 유효하지 않습니다.",
...
def updateTodo(
    schema: DeleteTodo, token=Depends(oauth2Scheme), db: Session = Depends(get_db)
):
    credentialsException = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="토큰이 유효하지 않습니다.",
